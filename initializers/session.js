    ...
    api.session = {
      ...
      load: function (connection, callback) {
        ...
        redis.get(key, function (error, data) {
          if (error) { return callback(error) } else if (data) { return callback(null, JSON.parse(data)) } else { return callback(null, false) }
        })
      },
        ...
        'logged-in-session': {
          ...
          preProcessor: function (data, callback) {
            api.session.load(data.connection, function (error, sessionData) {
              if (error) { return callback(error) } else if (!sessionData) {
